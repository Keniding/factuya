...
import { devTenant, peSunatAdapter } from "./config";
...
/**
 * Servidor HTTP real de `apps/api` — implementa el subconjunto de docs/sdd/factuya-sdd.md §7
 * que este MVP soporta de verdad (ver docs/flows.md para lo que falta). Usa Bun.serve (herramienta
 * de desarrollo, ver CLAUDE.md punto 4) en vez de la arquitectura de producción (API Gateway +
...
async function handleCreateInvoice(req: Request): Promise<Response> {
  ...
  let request;
  try {
    request = parseInvoiceRequest(rawBody);
  } catch (err) {
  ...
  const id = randomUUID();
  try {
    const result = await emitInvoice(peSunatAdapter, request, devTenant);
    saveInvoice(id, result);
...
function handleGetInvoice(id: string): Response {
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (method === "GET" && invoiceMatch) {
      return handleGetInvoice(invoiceMatch[1]!);
    }
...
console.log(`Tenant de desarrollo: RUC ${devTenant.issuer.taxId}, canal ${devTenant.submissionChannel}`);
